...
function getWinners(array, cb) {
  const winners = [];
  cb(array).forEach(function (item) {
...
function getWinnersByYear(array, cb1, cb2) {
  const winnersByYear = [];
  const years = cb1(array, getFinals);
  const winners = cb2(array, getFinals);
...
function getAverageGoals(cb) {
  const finalsArray = cb;
  const goalsTotal = finalsArray.reduce(function (accumulator, currentValue) {
    return (
      accumulator +
  ...
  return (goalsTotal / finalsArray.length).toFixed(2);
}
